...
- Signed `0.1.1`, `0.1.2`, `0.1.4`, `0.1.5`, `0.1.6`, `0.1.7`, `0.1.8`, `0.1.10`, `0.1.11`, `0.1.12`, `0.1.13`, `0.1.16`, and `0.1.17` artifacts are retained side-by-side across the accepted endpoints. Cloud now selects signed `0.1.17` with signed `0.1.16` as `previous`; this workstation still selects signed local `0.1.13` with signed `0.1.12` as `previous`. Immutable `0.1.9` remains local-only failed-release evidence and was never activated. A separately audited `/Users/BofeiChen` local endpoint was not part of that acceptance; it still selects `0.1.8/0.1.7` and has signed `0.1.12` plus `0.1.13` staged side-by-side for an explicitly approved later activation sequence.
...
Status: signed Cloudx `0.1.17` is active on cloud with `0.1.16` retained as N-1; local still selects `0.1.13`. The staged `.policy.1` candidates and never-staged `.policy.2` source candidates are superseded. Reproducible `.policy.3` candidates separate the two-request business ceiling from trigger-only high-concurrency incident diagnosis, retain the immediate direct-permanent receipt path, and add identity-free aggregate pool observations. Revised policies and dual watchers remain inactive and separately unapproved for activation.
...
- [ ] Install and activate signed Cloudx `0.1.17` locally while retaining signed N-1 and without restarting CPA or the active Codex communication path.
