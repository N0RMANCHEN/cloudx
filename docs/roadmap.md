...
Status: cloud selects signed Cloudx `0.1.21/0.1.20`; local still selects `0.1.20/0.1.19`. Exact cloud `.policy.4` is active with the signed failure/sweep watchers; local `.policy.4` is staged/inactive and superseded while the original local CPA and inactive watcher remain. Local `0.1.21` installation, `.policy.5` staging, cloud activation, repeated M4B acceptance, and zero-connection local activation remain separate gates.
...
- [ ] Install signed Cloudx `0.1.21` on cloud then local without restarting either CPA, retaining signed `0.1.20` and preserving communication/process/credential state.
