...
- A strict release-ordering evidence fixture evaluates Cloudx `0.1.13`/`0.1.12` against the recorded Phi current/N-1 across four release pairs, two upgrade orders, and both rollback directions. Phi current supports both Cloudx releases directly. Source `0.1.15` packages a bounded, secret-free formal-to-legacy health bridge fixed to an immutable artifact; its generated example is accepted by the exact Phi N-1 parser at release `17d3e42`, while unknown process/failure facts remain unknown. A deterministic isolated selector rehearsal passed, the exact-confirmation installer keeps all signed units inactive, and a new static canary can exercise the artifact under systemd while writing only a temporary `/run` document and masking the production legacy path. The audit still identifies `legacy_bridge_pending` until the bridge is signed/published, installed and primary-unit runtime-accepted, and production rollback-rehearsed, so the M4A ordering checkbox remains open.
...
The ordering verification now has an explicit compatibility path rather than a declaration: `config/governance/phi_cloudx_release_ordering.v1.json` binds exact Cloudx and Phi release refs, and `scripts/check_phi_cloudx_release_ordering.py` evaluates the complete pair/order matrix. `config/governance/phi_cloudx_legacy_health_bridge.v1.json` separately binds source `0.1.15`, the fixed-artifact primary/canary templates, both non-authorizing operator plans, the exact Phi N-1 consumer digest, and the accepted isolated selector round trip. Its checker executes the installer and canary read-only plans, requires every authorization bit to remain false, and rejects any canary path capable of publishing to `/var/lib/cloudx/health`; its checkout-aware mode classified the bridge example as degraded/low-capacity rather than invalid, incompatible, or stale. The first unchecked item above remains open on three explicit production gates: signed artifact publication, primary bridge unit runtime acceptance, and independent runtime rollback rehearsal.
