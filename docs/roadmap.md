...
Status: signed Cloudx `0.1.20` is active on cloud and local with signed `0.1.19` retained. Cloud `.policy.3` and its signed failure/sweep watcher transaction remain active; local `.policy.3` and local watcher remain staged/inactive after the historical failed restart. Signed `0.1.20` carries independently reproducible `.policy.4` candidates that trigger incident calibration for final `auth_unavailable` or all-candidate `model_cooldown` without changing any archive condition. Both stages, cloud activation, repeated acceptance, and zero-connection local activation remain separate gates.
...
- [ ] Stage both exact `.policy.4` candidates without changing a process, launcher, unit, credential, archive, watcher, or release selector.
