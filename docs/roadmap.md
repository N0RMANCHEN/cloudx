...
- The first deferred local `.policy.3` activation job failed at `2026-07-18 23:16 CST` while many existing CPA requests were active. Baseline shutdown logged an HTTP deadline overrun; the old rollback path raced launchd unloading and left the service offline even though it restored the original launcher bytes. The operator manually reopened the unchanged baseline at `23:54 CST`; current baseline health is accepted. Repository `0.1.18` now prepares the manual recovery implementation before scheduling, refuses launcher mutation until five zero-connection samples pass, makes automatic recovery use the exact same command, records secret-free stages and service availability, and includes an isolated executable recovery rehearsal. No second production restart is authorized by this correction.
...
- [ ] Publish and install signed Cloudx `0.1.18` locally, then activate local `.policy.3` only through a new separate confirmation after the recovery bundle and zero-connection gate pass; accept health, two-request business-policy, real communication, trigger emission, and manual/automatic recovery canaries while retaining original binary and snapshot.
