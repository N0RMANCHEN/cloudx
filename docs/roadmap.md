...
- Signed releases through `0.1.19` are retained immutably. Cloud and this workstation still select signed `0.1.18` with signed `0.1.17` as `previous`; signed `0.1.19` is published but not installed on either real endpoint. Immutable `0.1.9` remains local-only failed-release evidence and was never activated. A separately audited `/Users/BofeiChen` local endpoint was not part of that acceptance; it still selects `0.1.8/0.1.7` and has signed `0.1.12` plus `0.1.13` staged side-by-side for an explicitly approved later activation sequence.
...
Status: signed Cloudx `0.1.19` is published but not installed; both endpoints remain on signed `0.1.18/0.1.17`. The staged `.policy.1` candidates and never-staged `.policy.2` source candidates are superseded. Cloud `.policy.3` and its signed failure/sweep watcher transaction are active; idle fallback performs zero probes. The first deferred local policy attempt failed during a non-quiescent graceful shutdown and left the baseline offline until manual operator recovery; local `.policy.3` and local watcher therefore remain staged/inactive. The production-discovered non-refreshable-401 sweep defect is fixed in signed `0.1.19`; any future local activation requires signed local `0.1.19`, repeated zero-connection proof, and its prebuilt independently executable recovery bundle.
...
- [ ] Install signed Cloudx `0.1.19` on cloud and local with signed `0.1.18` retained before repeating M4B acceptance or activating the local policy/watcher.
