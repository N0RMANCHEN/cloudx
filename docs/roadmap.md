...
Status: signed Cloudx `0.1.18` is active on cloud with signed `0.1.17` retained; local remains signed `0.1.17/0.1.13` pending its approved no-reseed install. The staged `.policy.1` candidates and never-staged `.policy.2` source candidates are superseded. Cloud `.policy.3` is active after a successful exactly confirmed retry. The first deferred local attempt failed during a non-quiescent graceful shutdown and left the baseline offline until manual operator recovery; local `.policy.3` therefore remains staged and inactive. A future attempt requires signed local Cloudx `0.1.18`, repeated zero-connection proof, and its prebuilt independently executable recovery bundle. Both watcher transactions remain inactive.
...
- [ ] Install signed Cloudx `0.1.18` locally through the approved signed no-reseed path, retain signed `0.1.17`, and preserve native profile, CPA/listener, launchd, credentials, archives, and all existing Codex communication.
