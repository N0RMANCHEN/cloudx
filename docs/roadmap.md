...
- Repository development is now `0.1.16`. Immutable annotated `v0.1.15` pins source `9ffa3208f39053c2b3af1136a530ce98eac7ad41`; successful tag workflow `29579921061` published artifact ref `332cb865a97d654efca4b4321b90cdc140e57e64`, moved stable to `78fe78303f9d57c592b103e11c0fdca1c373b37c`, and created a non-draft/non-prerelease GitHub Release with seven assets. Fresh clones accepted the replacement signer, rejected the old signer, matched every release asset, and staged local/cloud bundles idempotently in selector-free isolated roots. No real endpoint was staged or activated and no service was restarted. Immutable `v0.1.14` remains failed-publication history with no artifact ref or stable selection.
...
- Source `0.1.16` now adds the separately gated CPA credential-failure and concurrency safety batch. Cloudx local/cloud consumers accept only fresh `cloudx.cpa-auth-failure.v1` receipts bound to the current top-level auth-file SHA-256 and reversibly archive confirmed permanent auth failures; weekly quota, ordinary 429, transient/network/timeout/5xx failures, and expired access tokens with refresh tokens are never receipt authority. Exact-commit patches for the deployed local v7.0.1 and cloud v7.2.71 CPA revisions enforce one process-global maximum of two proxied API requests, preserve the local fast-service-tier mapping, and build reproducibly to pinned Darwin/arm64 and Linux/amd64 bytes. An isolated local candidate on non-production loopback port `18427` held two partial requests, kept a third completed request waiting until one slot was released, and returned the public maximum-two policy header on all three responses; the temporary listener was stopped. After the exact separate `STAGE LOCAL ...` and `STAGE CLOUD ...` confirmations, the candidates are now durably staged side-by-side at `/Users/hirohi/.local/lib/cliproxy-cloudx/releases/7.0.1-codexx-fast-service-tier-cloudx-policy.1` and `/opt/cliproxy-cloudx/releases/7.2.71-cloudx-policy.1`; both manifests and staged bytes match their pinned SHA-256 values. The stage outputs reported no external-service restart, local CPA remains PID `38189` on the baseline `/Users/hirohi/.local/bin/cli-proxy-api`, and cloud `cliproxy.service` remains PID `977036` with restart count `0` on `/usr/local/bin/cli-proxy-api`. No launcher/unit, credential, archive, Cloudx selector, or active binary changed, so the two-request ceiling and failure receipts are not active in production yet; activation remains separately unapproved.
- A separately authorized cloud import normalized the exact OpenAI OAuth CPA-export wrapper and atomically wrote ten mode-`0600` records; an idempotent repeat reported ten skipped and zero writes. Real cloud Codex traffic then proved the records all represented one `k12` workspace with no refresh token and returned explicit `deactivated_workspace`, followed by `auth_unavailable`; cloud CPA PID `977036` and restart count `0` remained unchanged. Cloud write acceptance is therefore not counted as usable fallback capacity, and the imported records were not manually removed or archived. A separate direct `soul0` official-Codex canary succeeded without either CPA, and the current local `api` profile real-Codex baseline canary also succeeded. Source now accepts only the exact `platform=openai` OAuth wrapper, rejects another platform, and requires real local Codex communication before/after activation and after rollback through a deferred current-turn-safe worker.
...
Status: source and deterministic candidates ready; both CPA candidates are durably staged and inactive, while signed Cloudx rollout plus CPA activation remain separately unapproved.
...
- [x] Define strict `cloudx.cpa-auth-failure.v1` evidence with two confirmations, a conclusive enumerated permanent-auth reason, fixed `weeklyQuota=false`, a fresh observation window, and exact auth-file digest binding.
...
- [x] Pin exact CPA patches and deterministic candidate identities for Darwin/arm64 and Linux/amd64; preserve the local `fast -> priority` service-tier compatibility and enforce one process-global two-request ceiling with waiting backpressure.
...
- [ ] Publish and activate a signed Cloudx release containing the receipt consumers on cloud first and local second, retaining signed N-1 and updating only the separately approved CPA-health unit path contract.
...
- Local activation is attempted synchronously from a CPA-backed Codex turn, before signed Cloudx `0.1.16` is active, or without successful real communication canaries before candidate selection, after candidate selection, and after any rollback.
