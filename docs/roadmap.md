...
- A strict release-ordering evidence fixture evaluates Cloudx `0.1.13`/`0.1.12` against the recorded Phi current/N-1 across four release pairs, two upgrade orders, and both rollback directions. Phi current supports both Cloudx releases directly. Source `0.1.15` now packages a bounded, secret-free formal-to-legacy health bridge fixed to an immutable artifact; its generated example is accepted by the exact Phi N-1 parser at release `17d3e42`, while unknown process/failure facts remain unknown. The audit now identifies `legacy_bridge_pending` rather than a bare schema mismatch, but truthfully stays `blocked` until the bridge is signed/published, installed side-by-side, and rollback-rehearsed, so the M4A ordering checkbox remains open.
...
The ordering verification now has an explicit compatibility path rather than a declaration: `config/governance/phi_cloudx_release_ordering.v1.json` binds exact Cloudx and Phi release refs, and `scripts/check_phi_cloudx_release_ordering.py` evaluates the complete pair/order matrix. `config/governance/phi_cloudx_legacy_health_bridge.v1.json` separately binds source `0.1.15`, the fixed-artifact templates, and the exact Phi N-1 consumer digest. Its checkout-aware checker executed that parser successfully and classified the example as degraded/low-capacity rather than invalid, incompatible, or stale. The first unchecked item above remains open on three explicit runtime gates: signed artifact publication, bridge unit acceptance, and independent rollback rehearsal.
