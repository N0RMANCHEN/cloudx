...
- Signed `0.1.1`, `0.1.2`, `0.1.4`, `0.1.5`, `0.1.6`, `0.1.7`, `0.1.8`, `0.1.10`, `0.1.11`, and `0.1.12` artifacts are staged side-by-side on both endpoints. Both `current` links select signed `0.1.11`, and both `previous` links select signed `0.1.10`; immutable `0.1.9` remains local-only failed-release evidence and was never activated.
...
- Repository development has advanced to `0.1.13` after immutable signed `0.1.12` published and staged on both endpoints without activation. Signed `0.1.11` remains active while the `0.1.12` shell-path hardening awaits a separate activation and rollback decision.
...
- Fresh-shell acceptance on this workstation confirms signed `0.1.11` keeps real `HOME=/Users/hirohi`, resolves official Codex directly, and supports `codexx api`, compatibility `codexx use api`, named accounts, and `codexx exit`. An active codex-plus process regenerated the API account's legacy Git shim, proving deletion remains unsafe. Signed staged `0.1.12` removes only account-local `.local/bin` entries when its hook is loaded from a legacy environment, restoring official Git without changing account records, shell configuration, the shim, the local CPA, or existing Codex processes; it is not active yet.
...
- Signed `0.1.12` was built from commit `1f94816d7effc62bb3199158066c1ed0b6ef38a6`, published to immutable `release-artifacts/v0.1.12`, selected by the signed stable ref, and staged idempotently on both endpoints without changing active or previous links.
...
- [x] Validate native return, API compatibility selection, named-account selection, official Codex resolution, and private recovery in isolated fresh shells; harden the next source hook against regenerated account-local Git shims without editing the active shim.
