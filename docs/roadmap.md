...
Status: signed Cloudx `0.1.17` is active on cloud and local with signed N-1 retained. The staged `.policy.1` candidates and never-staged `.policy.2` source candidates are superseded. Both `.policy.3` candidates are durably staged and inactive. The revised policies separate the two-request business ceiling from trigger-only high-concurrency incident diagnosis, retain the immediate direct-permanent receipt path, and add identity-free aggregate pool observations. Cloud-first policy activation and dual watchers remain separately unapproved.
