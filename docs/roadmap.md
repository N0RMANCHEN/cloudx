...
Status: cloud and local both select signed Cloudx `0.1.21/0.1.20`. Exact cloud `.policy.5` is active with signed failure/sweep watchers; exact local `.policy.5` is staged/inactive while the original local CPA and inactive watcher remain. Repeated M4B acceptance and zero-connection local activation remain separate gates.
...
- [ ] Repeat M4B production acceptance against the natural typed all-candidate cooldown trigger.
