...
- The initial Phi Personal Agent Mesh topology is frozen by the authoritative product contract and a machine-checked governance profile. The signed cloud artifact now also publishes `cloudx.phi-mesh-compatibility-profile.v1`, a secret-free, read-only reference to the existing handshake, health, gateway, client-configuration, signed-release, and rollback contracts. Devices still terminate at Phi cloud, only Phi cloud is a normal Cloudx Mesh consumer, and neither item authorizes a runtime, credential, service, release, or activation change.
