...
- Signed `0.1.1`, `0.1.2`, `0.1.4`, `0.1.5`, `0.1.6`, `0.1.7`, `0.1.8`, and `0.1.10` artifacts are staged side-by-side on both endpoints. Both `current` links select signed `0.1.10`, and both `previous` links select signed `0.1.8`; immutable `0.1.9` remains local-only failed-release evidence and was never activated.
...
- Signed `0.1.7` introduced the active account-state and `cloudx.health.v1` publishers. Signed `0.1.8` remains N-1 rollback, while signed `0.1.10` executes the enabled publishers and repeatedly publishes `/run/cloudx-account-state/accounts.json` and `/run/cloudx/health.json`; the legacy health contract remains active as rollback.
- Repository development has advanced to `0.1.11` after signed `0.1.10` activated the native CPA auth scanner, quota probe, atomic quarantine, explicit restore path, and recovered trust root on both endpoints.
- Phi's first consumer migration is active: `phi-cloudx-health` runs from its own versioned release, reads `/run/cloudx/health.json` with its restricted identity, and repeats on its timer without credential access. It successfully consumes producer `0.1.10` and currently reports degraded capacity because five accounts are quota-limited, while gateway and import dependencies remain ready.
- The signed `0.1.10` CPA-health unit is active and executes from `/opt/cloudx/current/cloudx-cloud.pyz` without `/opt/codex-gateway/codexx_app` in its service contract. Two natural runs returned aggregate-only healthy state with 15 total, 10 ready, 5 limited, 0 failed, and 0 archived; auth/archive inventories remained unchanged. The old unit and private state remain in root-only rollback snapshot `/var/lib/cloudx/cpa-health-service-backups/20260715T130117Z-0.1.10`.
- M5 dependency preflight still finds the legacy `18317` listener on the other macOS endpoint, this workstation's local CPA and existing Codex sessions, the old cloud HTTP importer, legacy health exporter, and `/opt/codex-gateway/codexx_app`. Cloudx CPA health no longer imports that runtime, but the active `/opt/codex-gateway/cloud_import_api.py` still imports `codexx_app.cloud_import_server`; the runtime cannot be retired before the old importer and its rollback consumers are separately handled. The quota-monitor and import-repair timers remain disabled. No broad deletion gate is accepted yet.
...
- Repository `0.1.11` now carries a signed-artifact `codex-gateway-import` compatibility adapter that routes FILE/stdin imports directly to `cloudx-remote import`, preserves `--force`, adds `--dry-run`, and contains no HTTP token or port `8780` dependency. It is source-ready only; installing the adapter and stopping the old importer remain separate gates.
...
- The `/Users/hirohi` macOS workstation now runs signed local `0.1.10` with signed `0.1.8` as `previous`. Its external local CPA remains PID `38189` on `127.0.0.1:8317`, all pre-existing Codex processes survived installation, activation, and rollback, and the private codex-plus recovery command remains healthy; this does not advance the broad M5 deletion gate.
