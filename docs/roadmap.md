...
- Both exact `.policy.4` candidates are staged and inactive after separate confirmations. Cloud staged root-owned SHA-256 `3e3ed137ff90132203f2b0e969245b6580b3ff2b780e2f3a47b821642fd6fdc4`; local staged owner-only SHA-256 `08608c2ebba606115a5c4bf6588896af3d2bdeb6e71ed308e17a84148766cd29`. Exact runtime identities, manifests, sizes, selectors, active CPA selections, cloud CPA/importer, local CPA and all six captured Codex PIDs, cloud credential/archive/watcher inputs, the 37-file local auth aggregate, and launcher/unit bytes remained unchanged. Cloud `.policy.3` is still active; local remains on its original CPA binary.
...
Status: signed Cloudx `0.1.20` is active on cloud and local with signed `0.1.19` retained. Both exact `.policy.4` candidates are staged and inactive. Cloud `.policy.3` and its signed failure/sweep watcher transaction remain active; local remains on the original CPA binary and its watcher remains inactive. Cloud `.policy.4` activation, repeated acceptance, and zero-connection local activation remain separate gates.
...
- [ ] Activate cloud `.policy.4` through its separate recovery-bounded confirmation and repeat the M4B production acceptance.
