...
Status: cloud and local both select signed Cloudx `0.1.21/0.1.20`. Exact `.policy.5` candidates are staged on both endpoints; cloud `.policy.4` remains active with signed failure/sweep watchers, while the original local CPA and inactive watcher remain. Cloud `.policy.5` activation, repeated M4B acceptance, and zero-connection local activation remain separate gates.
...
- [ ] Activate cloud `.policy.5` through a new independently recoverable `.policy.4` restart boundary and accept exact identity, health, policy `2`, real model traffic, and local communication.
