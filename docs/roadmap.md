...
- Repository development is now `0.1.17`. Immutable annotated `v0.1.16` pins source `ec77369a990418f2a990874d1d7bd4b9d2c7fe04`; successful tag workflow `29640659405` published artifact ref `9513ff87b3b2e45d2b3609f0746248a7422d34b2`, moved stable to `bba9f619fc2d3e57cbd1b2808fe97ac58e805aef`, and created a non-draft/non-prerelease GitHub Release with seven assets. Fresh clones accepted the current signer, rejected the previous signer, matched every release asset, and staged local/cloud bundles idempotently in selector-free isolated roots. No real endpoint was staged or activated and no service was restarted. Immutable signed `v0.1.15` remains the prior published release; `v0.1.14` remains failed-publication history with no artifact ref or stable selection.
...
- Signed `0.1.16` carries the first separately gated CPA credential-failure and concurrency safety batch. Its `.policy.1` exact-commit candidates remain durably staged but inactive at `/Users/hirohi/.local/lib/cliproxy-cloudx/releases/7.0.1-codexx-fast-service-tier-cloudx-policy.1` and `/opt/cliproxy-cloudx/releases/7.2.71-cloudx-policy.1`; they are now superseded and must not be activated. Source `0.1.17` retains the process-global maximum of two proxied API requests and local fast-service-tier mapping while adding infrastructure-gated account probing capped at two, bounded `deactivated_workspace` classification, immediate digest-bound archive after one conclusive permanent result, and event-driven network-free receipt consumers for local and cloud. Independently rebuilt `.policy.2` candidates match local SHA-256 `f288838053f43a82c50d2ab23bcb096c627a848fdf662413544a483f908f236d` and cloud SHA-256 `7c9603a380f9fbd7bdbe1c8ecbf938504f6055677ba4d4de2cd7004398a02229`; neither is staged or active, and both require signed Cloudx `0.1.17` before activation. Failure watchers also remain inactive behind their own exact confirmations and require both the signed consumer and matching policy producer first.
...
Status: signed Cloudx `0.1.16` is active on cloud and local remains `0.1.13`; the staged `.policy.1` candidates are superseded and inactive. Source `0.1.17` plus reproducible `.policy.2` candidates implement corrected bounded-two/direct-permanent semantics and event-driven receipt consumption, but the release, revised policies, and watchers are unpublished or inactive and remain separately unapproved for activation.
...
- [x] Pin revised exact CPA patches and deterministic `.policy.2` identities for Darwin/arm64 and Linux/amd64; recognize `deactivated_workspace`, emit on the first conclusive permanent failure, preserve the local `fast -> priority` service-tier compatibility, and enforce one process-global two-request ceiling with waiting backpressure.
...
- [x] Add an infrastructure/provider gate, explicit external proxy input, account probing capped at two, bounded permanent-error parsing, direct conclusive archive, and Phi read-only boundary; retain quota/429, refreshable 401, network/TLS/DNS/timeout/5xx, and unknown failures.
- [x] Add a network-free receipt-only cloud command, release the archive lock during network probes, package signed systemd path/service templates, and provide a separate rollback-protected watcher activation transaction. Local uses `WatchPaths` with a two-minute fallback; cloud uses the path trigger with the five-minute full probe retained as fallback; neither watcher action restarts CPA or Codex.
- [ ] Publish signed Cloudx `0.1.17`, then activate it cloud-first and local-second while retaining signed N-1 and without restarting either CPA.
- [x] Retain the previously staged `.policy.1` candidates as inactive evidence and mark them superseded; independently build matching `.policy.2` bytes twice from both exact upstream commits.
- [ ] Stage the `.policy.2` cloud and local candidates only after their distinct exact `STAGE ... CPA POLICY ...` confirmations.
- [ ] Activate `.policy.2` cloud then local only after signed Cloudx `0.1.17` is active on the matching endpoint; accept health, concurrency-policy, real communication, and rollback canaries while retaining original binaries and snapshots.
- [ ] Activate the cloud then local failure watchers through their separate exact confirmations only after each `.policy.2` receipt producer is live; verify path-triggered archive latency, periodic fallback, unit/launcher rollback, and unchanged CPA/Codex processes.
...
- [ ] Prove with natural traffic that concurrent proxied requests never exceed two, weekly quota creates no archive, one provisional refreshable 401 creates no archive, one conclusive permanent refresh/auth result archives exactly one digest-matched credential, and exact restore returns it safely.
...
- A failure watcher is activated before its exact signed consumer and matching `.policy.2` producer, performs a network request, or restarts CPA, Codex, Cloudx, or Phi.
