...
Status: cloud and local both select signed Cloudx `0.1.21/0.1.20`. Exact cloud `.policy.4` is active with the signed failure/sweep watchers; local `.policy.4` is staged/inactive and superseded while the original local CPA and inactive watcher remain. `.policy.5` staging, cloud activation, repeated M4B acceptance, and zero-connection local activation remain separate gates.
...
- [ ] Stage both exact `.policy.5` candidates through separate confirmations without changing a process, service, launcher, unit, credential, archive, watcher input, or release selector.
