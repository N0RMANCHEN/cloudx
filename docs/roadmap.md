...
- Signed `0.1.17` carries the corrected CPA credential-failure and concurrency safety batch and is active on both accepted endpoints with signed N-1 retained. Its superseded `.policy.1` candidates remain durably staged but inactive, while `.policy.2` was never staged. The signed release retains the process-global maximum of two proxied business API requests and local fast-service-tier mapping, while a final aggregate `auth_unavailable` emits an identity-free incident trigger. Only that trigger starts an infrastructure-gated, duplicate-deduplicated account sweep with separate adaptive concurrency up to 32. Direct conclusive permanent receipts still archive immediately through a network-free path, and quota/429, provisional refreshable 401, transport/provider/5xx, and unknown failures remain in place. Both exact `.policy.3` candidates are now durably staged and inactive: cloud SHA-256 `453df72d15235ea51e5fdf66d27692bb5249bd262800fd628af3638246021a2b`, local SHA-256 `1cff3152e34666d2753add54ce7f5f96dbd643e607c1f136a9052cd28eba9ecd`. The first exactly confirmed cloud activation selected the correct candidate but its one-shot canary raced socket readiness in the same second; automatic rollback restored the original binary, unit selection, healthy listener, and absent policy drop-ins. Source now uses bounded readiness retries, explicit rollback verification, and safe cleanup of activation-created empty directories; `.policy.3` remains inactive and any retry requires a new exact confirmation. The unchanged cloud health unit still invokes the pre-watcher command, but with zero active accounts its accepted natural run performed zero probes; trigger-aware timer fallback must be installed with the separate watcher transaction before usable capacity is imported.
...
Status: signed Cloudx `0.1.17` is active on cloud and local with signed N-1 retained. The staged `.policy.1` candidates and never-staged `.policy.2` source candidates are superseded. Both `.policy.3` candidates are durably staged and inactive. The first exactly confirmed cloud activation safely rolled back after a readiness race; its retry, local activation, and dual watchers remain separately unapproved. The revised policies separate the two-request business ceiling from trigger-only high-concurrency incident diagnosis, retain the immediate direct-permanent receipt path, and add identity-free aggregate pool observations.
