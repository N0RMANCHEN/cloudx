...
- Repository development is now `0.1.21`. Immutable annotated `v0.1.20` pins source `eefd8a4aa70d554ac32babe3ff7aa1ae9996e875`; main CI `29679388154`, non-publishing signing canary `29679446529`, and tag workflow `29679495793` passed. Publication created artifact ref `d6bce9346d86b2b133057df01ced068ef3eef9b2`, moved stable to `11edbaf440d0f38958743aea279ddc36ea9320e5`, and created final GitHub Release `356290726` with seven assets. Fresh immutable clones/downloads accepted the current signer, rejected the pre-rotation signer, matched every asset and offline member, passed both self-checks and stable selection, and staged local/cloud bundles idempotently without selectors. Both real endpoints remain `0.1.19/0.1.18`; local CPA remains PID `61859`, cloud CPA remains PID `1613475` with restart count `0`, importer PID `133756` with restart count `0`, one active cloud credential, 45 archive entries, and empty failure/trigger inputs.
...
- Rollback-bounded transaction `20260719T090507Z-14915b32` repeated every isolated M4B boundary under active `.policy.4` but safely rejected natural all-candidate cooldown with `aggregate_not_triggered`. Recovery restored one useful credential, 45 archive entries, empty watcher inputs, HTTP `200` policy-`2` traffic, cloud CPA PID `1693505`/restart count `0`, local CPA PID `61859`, and captured Codex processes. Exact upstream returns private `modelCooldownError`, not `*coreauth.Error`, making `.policy.4`'s later code-string branch unreachable. Source `0.1.21` now carries deterministic `.policy.5` candidates using a typed capability on the real error before the cast; focused regressions exercise the real constructor, and three exact builds match local SHA-256 `bb6fe9cfcc26d521ce0dcf9f503d2dffa742bce62bd359cab8f91052116c0db3` and cloud SHA-256 `5f83b1821d2be7cf5b7615973e4e6130d477386e16eae3a50af46e99bf7af7f8`. Neither candidate is staged or active.
...
Status: signed Cloudx `0.1.20` is active on cloud and local with signed `0.1.19` retained. Exact cloud `.policy.4` is active with the signed failure/sweep watchers; local `.policy.4` is staged/inactive and superseded while the original local CPA and inactive watcher remain. Source `0.1.21` contains independently reproducible `.policy.5` candidates, but publication, endpoint installation, staging, cloud activation, repeated M4B acceptance, and zero-connection local activation remain separate gates.
...
- [ ] Publish signed Cloudx `0.1.21` without endpoint activation and accept immutable refs, signatures, assets, offline parity, self-checks, and selector-free staging evidence.
