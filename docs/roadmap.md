...
Status: signed Cloudx `0.1.18` is published but not installed; signed `0.1.17` remains active on cloud and local with signed N-1 retained. The staged `.policy.1` candidates and never-staged `.policy.2` source candidates are superseded. Cloud `.policy.3` is active after a successful exactly confirmed retry. The first deferred local attempt failed during a non-quiescent graceful shutdown and left the baseline offline until manual operator recovery; local `.policy.3` therefore remains staged and inactive. A future attempt requires signed local Cloudx `0.1.18`, repeated zero-connection proof, and its prebuilt independently executable recovery bundle. Both watcher transactions remain unapproved.
...
- [ ] Install signed Cloudx `0.1.18` cloud-first then local through separate confirmations, retaining signed N-1 and preserving both CPA processes, listeners, units, launchers, credentials, archives, and existing Codex communication.
