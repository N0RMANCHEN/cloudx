...
Status: signed Cloudx `0.1.21` is published but not installed; both endpoints still run `0.1.20/0.1.19`. Exact cloud `.policy.4` is active with the signed failure/sweep watchers; local `.policy.4` is staged/inactive and superseded while the original local CPA and inactive watcher remain. Cloud-first/local-second `0.1.21` installation, `.policy.5` staging, cloud activation, repeated M4B acceptance, and zero-connection local activation remain separate gates.
