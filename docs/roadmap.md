...
- Repository development has advanced to `0.1.9`; its cloud candidate now contains the native CPA auth scanner, quota probe, atomic quarantine, and explicit restore path. Deployed endpoints still run signed `0.1.8` and retain signed `0.1.7` for rollback.
...
- M5 dependency preflight still finds the legacy `18317` listener, local CPA, existing Codex sessions, old cloud importer, legacy health exporter, and `/opt/codex-gateway/codexx_app` compatibility runtime. The installed signed `0.1.8` CPA-health unit still loads that runtime; the unsigned `0.1.9` source candidate removes the dependency and passed read-only parity, but has not been published or activated. The quota-monitor timer was disabled by a concurrent external change outside the Cloudx cutover. No deletion gate is accepted yet.
