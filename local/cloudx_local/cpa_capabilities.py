...
SCHEMA = "cloudx.local-cpa-capabilities.v1"
...
def _manifest(config: LocalConfig) -> Tuple[Dict[str, Any], pathlib.Path]:
    configured = config.local_cpa_capability_manifest
...
def _binary(config: LocalConfig, document: Dict[str, Any]) -> Tuple[pathlib.Path, str]:
    configured = config.local_cpa_binary
    ...
    path = pathlib.Path(configured).expanduser() if configured is not None else declared
...
def _probe_url(config: LocalConfig) -> str:
    value = str(config.local_cpa_capability_probe_url or "").strip()
    try:
...
def attest(
    ...
    *,
    ...
    document, unused_manifest_path = _manifest(config)
    unused_binary_path, actual_digest = _binary(config, document)
