...
- Signed releases through `0.1.20` are retained immutably. Cloud and this workstation still select signed `0.1.19/0.1.18`; publication did not install either endpoint. Immutable `0.1.9` remains local-only failed-release evidence and was never activated. A separately audited `/Users/BofeiChen` local endpoint was not part of that acceptance; it still selects `0.1.8/0.1.7` and has signed `0.1.12` plus `0.1.13` staged side-by-side for an explicitly approved later activation sequence.
...
Status: signed Cloudx `0.1.20` is published but not installed; cloud and local remain on signed `0.1.19/0.1.18`. Cloud `.policy.3` and its signed failure/sweep watcher transaction remain active; local `.policy.3` and local watcher remain staged/inactive after the historical failed restart. Signed `0.1.20` carries independently reproducible `.policy.4` candidates that trigger incident calibration for final `auth_unavailable` or all-candidate `model_cooldown` without changing any archive condition. Endpoint installation, staging, cloud activation, repeated acceptance, and zero-connection local activation remain separate gates.
...
- [ ] Install signed Cloudx `0.1.20` cloud-first then local with signed `0.1.19` retained, and stage both exact `.policy.4` candidates without changing a process, launcher, unit, credential, archive, watcher, or release selector.
