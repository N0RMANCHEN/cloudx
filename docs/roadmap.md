...
- Source `0.1.15` now includes a default-offline, exact-confirmation transaction for synchronizing the replacement key into the fixed GitHub `release` environment and proving it through a non-publishing `workflow_dispatch`. It binds the key to the committed root and pushed `main`, passes key bytes only through GitHub CLI stdin, requires the signing canary plus unchanged tag/artifact/stable refs, and cannot falsely claim rollback after GitHub accepts a secret write. This endpoint is not authenticated to GitHub CLI, so only the path-free non-authorizing plan ran; no GitHub secret, workflow run, tag, artifact ref, stable ref, endpoint, or service changed.
