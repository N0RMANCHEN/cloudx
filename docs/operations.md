...
The replacement-workstation transaction is now accepted. Two unusable orphaned exec groups retired with `SIGTERM` only; the zero-connection legacy control service moved to the verified retained runtime through one bounded port-`8765` reload; and the three live package targets entered private quarantine `20260719T150428Z`. Control rollback backup `20260719T150309Z`, retained-runtime recovery, and package recovery all passed. External CPA PID `61859` and port `8317`, signed `0.1.21/0.1.20` selectors, and three real official-Codex communication canaries remained continuous. See the archived production acceptance for the secret-free evidence.
