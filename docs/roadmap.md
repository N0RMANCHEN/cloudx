...
- Signed `0.1.1`, `0.1.2`, `0.1.4`, `0.1.5`, `0.1.6`, `0.1.7`, `0.1.8`, `0.1.10`, `0.1.11`, `0.1.12`, and `0.1.13` artifacts are staged side-by-side on both endpoints. Both `current` links select signed `0.1.12`, and both `previous` links select signed `0.1.11`; immutable `0.1.9` remains local-only failed-release evidence and was never activated.
...
- Signed `0.1.7` introduced the active account-state and `cloudx.health.v1` publishers. Signed `0.1.10` introduced the native CPA implementation, while signed `0.1.12` now executes the enabled publishers and repeatedly publishes `/run/cloudx-account-state/accounts.json` and `/run/cloudx/health.json`; signed `0.1.11` is N-1 and the legacy health contract remains active as rollback.
- Repository development has advanced to `0.1.14` after immutable signed `0.1.13` published and staged on both endpoints without activation. Signed `0.1.12` remains active while the `0.1.13` whitespace-idempotent hook installer and detached read-only SSH stdin handling await a separate activation and rollback decision.
- An isolated exact-signed `0.1.13 -> 0.1.12 -> 0.1.13` dual-endpoint rehearsal passed with final `current=0.1.13`, `previous=0.1.12`, canonical idempotent hook bytes, and a signed scripted cloud-mode canary. Because the first local cutover runs under current `0.1.12`, the production plan must immediately repeat the same-version local `0.1.13` apply so the newly current updater removes inherited blank-line drift. A `/tmp` path alias also exposed an N-1 comparison defect that does not affect canonical production `/Users` or `/opt` roots; source `0.1.14` now preserves `previous` across aliased same-version apply and has regression coverage.
- Phi's first consumer migration is active: `phi-cloudx-health` runs from its own versioned release, reads `/run/cloudx/health.json` with its restricted identity, and repeats on its timer without credential access. It accepted producer `0.1.11`; the Cloudx producer now selects protocol-compatible `0.1.12`, and no Phi service was restarted during activation.
- The native CPA-health unit installed from signed `0.1.10` remains active and executes the current signed `0.1.12` artifact from `/opt/cloudx/current/cloudx-cloud.pyz` without `/opt/codex-gateway/codexx_app` in its service contract. Its accepted `0.1.10` observation returned aggregate-only healthy state with 15 total, 10 ready, 5 limited, 0 failed, and 0 archived; auth/archive inventories remained unchanged. The old unit and private state remain in root-only rollback snapshot `/var/lib/cloudx/cpa-health-service-backups/20260715T130117Z-0.1.10`.
...
- Fresh-shell acceptance on this workstation confirms active signed `0.1.12` keeps real `HOME=/Users/hirohi`, removes only inherited account-local `.local/bin` entries, preserves Codex temporary paths, resolves official Git and Codex directly, and supports `codexx api`, compatibility `codexx use api`, named accounts, cloud mode, and `codexx exit`. The exact `0.1.12 -> 0.1.11 -> 0.1.12` local rollback reinstalled each release's signed hook and passed in clean shells. Signed staged `0.1.13` adds whitespace-idempotent hook installation and safe scripted cloud selection but is not active. The regenerated API Git shim and active codex-plus parents remain untouched, so deletion is still unsafe.
...
- The `/Users/hirohi` macOS workstation now runs signed local `0.1.12` with signed `0.1.11` as `previous`. Its external local CPA remains PID `38189` on `127.0.0.1:8317`; local API and isolated cloud model canaries passed, all pre-existing codex-plus and Codex processes survived activation and rollback, and the private recovery command remains healthy. This completes the workstation build prerequisite but does not satisfy the broad M5 deletion gate while those legacy parents remain active.
...
- Signed `0.1.13` was built from commit `d50c7d7b3d1cc390dde7e443fdff05f317a65e54`, published to immutable `release-artifacts/v0.1.13`, selected by the signed stable ref, and staged idempotently on both endpoints without changing active or previous links.
