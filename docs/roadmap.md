...
- A strict release-ordering evidence fixture now evaluates Cloudx `0.1.13`/`0.1.12` against the recorded Phi current/N-1 across four release pairs, two upgrade orders, and both rollback directions. Protocol ranges overlap at v1, and Phi current supports both Cloudx releases, but Phi N-1 still consumes the legacy health contract; the audit truthfully remains `blocked` and the M4A compatibility checkbox stays open.
...
The ordering verification now has executable partial evidence rather than a declaration: `config/governance/phi_cloudx_release_ordering.v1.json` binds exact Cloudx and Phi release refs, and `scripts/check_phi_cloudx_release_ordering.py` evaluates the complete pair/order matrix. It currently blocks on Phi N-1's legacy health contract, so the first unchecked item above is intentionally not marked complete.
