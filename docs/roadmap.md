...
- Signed releases through `0.1.19` are retained immutably. Cloud and this workstation both select signed `0.1.19/0.1.18`. Immutable `0.1.9` remains local-only failed-release evidence and was never activated. A separately audited `/Users/BofeiChen` local endpoint was not part of that acceptance; it still selects `0.1.8/0.1.7` and has signed `0.1.12` plus `0.1.13` staged side-by-side for an explicitly approved later activation sequence.
...
- Repository development is now `0.1.20`. Immutable annotated `v0.1.19` pins source `51c5294ed6dd5b504ef4384e9860c70e2593ae78`; successful tag workflow `29677581155` published artifact ref `d4bddf35a1305186ffe568438ceaca6afb5cce61`, moved stable to `e5f7fafb7539b09f5b3e0f5424999bccd5a06dac`, and created non-draft/non-prerelease GitHub Release `356279554` with seven assets. Fresh clones accepted the current signer, rejected the pre-rotation signer, matched every asset and offline-bundle member, passed both self-checks, and staged local/cloud bundles idempotently in selector-free isolated roots. Publication changed no real endpoint: both remain `0.1.18/0.1.17`; local CPA remains PID `61859`, while cloud CPA remains PID `1613475`, restart count `0`, with one active credential and 45 prior archive entries. Immutable signed `v0.1.18` remains the prior published release.
...
Status: signed Cloudx `0.1.19` is active on cloud and local with signed `0.1.18` retained. Cloud `.policy.3` and its signed failure/sweep watcher transaction remain active; local `.policy.3` and local watcher remain staged/inactive after the historical failed restart. The active signed consumer correctly handles non-refreshable 401, and the recovered live acceptance proved every archive classification, but exact upstream expresses a fully quota-cooled pool as `model_cooldown`. Source `0.1.20` therefore supersedes `.policy.3` with independently reproducible `.policy.4` candidates that trigger incident calibration for final `auth_unavailable` or all-candidate `model_cooldown` without changing any archive condition. Publication, endpoint installation, staging, cloud activation, repeated acceptance, and zero-connection local activation remain separate gates.
...
- [ ] Publish and install signed Cloudx `0.1.20`, then stage both exact `.policy.4` candidates without changing a process, launcher, unit, credential, archive, watcher, or release selector.
