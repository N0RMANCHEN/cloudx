...
Status: signed Cloudx `0.1.19` is active on cloud and local with signed `0.1.18` retained. The staged `.policy.1` candidates and never-staged `.policy.2` source candidates are superseded. Cloud `.policy.3` and its signed failure/sweep watcher transaction are active; idle fallback performs zero probes. The first deferred local policy attempt failed during a non-quiescent graceful shutdown and left the baseline offline until manual operator recovery; local `.policy.3` and local watcher therefore remain staged/inactive. The production-discovered non-refreshable-401 sweep defect is fixed in the active signed consumer; local activation still requires repeated zero-connection proof and its prebuilt independently executable recovery bundle.
...
- [ ] Activate local `.policy.3` only through a new separate confirmation after the recovery bundle and zero-connection gate pass; accept health, two-request business-policy, real communication, trigger emission, and manual/automatic recovery canaries while retaining original binary and snapshot.
...
- [ ] Activate the local dual-input watcher only after local `.policy.3` is live; preserve CPA/Codex identities and verify immediate receipt consumption plus missed-trigger fallback without a CPA restart.
