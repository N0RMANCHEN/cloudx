...
- Signed `0.1.16` carries the first separately gated CPA credential-failure and concurrency safety batch. Its `.policy.1` exact-commit candidates remain durably staged but inactive at `/Users/hirohi/.local/lib/cliproxy-cloudx/releases/7.0.1-codexx-fast-service-tier-cloudx-policy.1` and `/opt/cliproxy-cloudx/releases/7.2.71-cloudx-policy.1`; they are superseded and must not be activated. The never-staged `.policy.2` source candidates are also superseded. Source `0.1.17` retains the process-global maximum of two proxied business API requests and local fast-service-tier mapping, while a final aggregate `auth_unavailable` now emits an identity-free incident trigger. Only that trigger starts an infrastructure-gated, duplicate-deduplicated account sweep with separate adaptive concurrency up to 32; ordinary timer runs perform no account probes. Direct conclusive permanent receipts still archive immediately through a network-free path, and quota/429, provisional refreshable 401, transport/provider/5xx, and unknown failures remain in place. Independently rebuilt `.policy.3` candidates match local SHA-256 `1cff3152e34666d2753add54ce7f5f96dbd643e607c1f136a9052cd28eba9ecd` and cloud SHA-256 `453df72d15235ea51e5fdf66d27692bb5249bd262800fd628af3638246021a2b`; neither is staged or active, and both require signed Cloudx `0.1.17` before activation. The dual failure/sweep watchers also remain inactive behind their own exact confirmations and require both the signed consumers and matching policy producer first.
...
Status: signed Cloudx `0.1.16` is active on cloud and local remains `0.1.13`; the staged `.policy.1` candidates and never-staged `.policy.2` source candidates are superseded. Source `0.1.17` plus reproducible `.policy.3` candidates separate the two-request business ceiling from trigger-only high-concurrency incident diagnosis, retain the immediate direct-permanent receipt path, and add identity-free aggregate pool observations. The release, revised policies, and dual watchers are unpublished or inactive and remain separately unapproved for activation.
...
- [x] Keep the network-free receipt-only fast path, add a distinct network-capable aggregate incident sweep, release the archive lock during probes, package both signed systemd path/service pairs, and provide one rollback-protected watcher activation transaction. Local watches both private inputs with a two-minute missed-trigger fallback; cloud watches both inputs with the five-minute timer reduced to trigger consumption only; neither watcher action restarts CPA or Codex.
...
- [ ] Stage the `.policy.3` cloud and local candidates only after their distinct exact `STAGE ... CPA POLICY ...` confirmations.
