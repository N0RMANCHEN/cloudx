...
Status: signed Cloudx `0.1.20` is active on cloud and local with signed `0.1.19` retained. Exact cloud `.policy.4` is active with the signed failure/sweep watchers; exact local `.policy.4` is staged/inactive while the original local CPA and inactive watcher remain. Repeated cloud M4B acceptance and zero-connection local activation remain separate gates.
...
- [ ] Repeat the M4B production acceptance against natural all-candidate `model_cooldown`.
- [ ] Activate local `.policy.4` only through a new separate confirmation after the recovery bundle and five-sample zero-connection gate pass; accept health, two-request business policy, real communication, both aggregate trigger sources, and manual/automatic recovery canaries while retaining original binary and snapshot.
...
- [ ] Activate the local dual-input watcher only after local `.policy.4` is live; preserve CPA/Codex identities and verify immediate receipt consumption plus missed-trigger fallback without a CPA restart.
...
- A watcher is activated before its exact signed consumers and matching `.policy.4` producer; the direct receipt consumer performs a network request; the aggregate sweep runs without a fresh trigger or skips its infrastructure gate; or either path restarts CPA, Codex, Cloudx, or Phi.
