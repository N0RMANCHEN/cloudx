...
Status: signed Cloudx `0.1.17` is published but not installed on either endpoint; cloud still selects `0.1.16` and local still selects `0.1.13`. The staged `.policy.1` candidates and never-staged `.policy.2` source candidates are superseded. Reproducible `.policy.3` candidates separate the two-request business ceiling from trigger-only high-concurrency incident diagnosis, retain the immediate direct-permanent receipt path, and add identity-free aggregate pool observations. Revised policies and dual watchers remain inactive and separately unapproved for activation.
...
- [ ] Install and activate signed Cloudx `0.1.17` cloud-first and local-second while retaining signed N-1 and without restarting either CPA.
