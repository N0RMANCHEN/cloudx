...
- Signed `0.1.1`, `0.1.2`, `0.1.4`, `0.1.5`, `0.1.6`, `0.1.7`, `0.1.8`, `0.1.10`, `0.1.11`, and `0.1.12` artifacts are staged side-by-side on both endpoints. Both `current` links select signed `0.1.12`, and both `previous` links select signed `0.1.11`; immutable `0.1.9` remains local-only failed-release evidence and was never activated.
...
- Repository development remains at `0.1.13` after immutable signed `0.1.12` activated cloud-first and local-second and completed a dual-endpoint N-1 rollback round trip. Source `0.1.13` additionally makes hook installation whitespace-idempotent and prevents read-only SSH probes from consuming scripted stdin; those source fixes are not installed yet.
...
- [x] Harden source `0.1.13` so repeated hook installation is whitespace-idempotent and read-only SSH probes detach from caller stdin while import and release-stage payloads retain explicit byte pipes.
