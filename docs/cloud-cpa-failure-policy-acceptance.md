...
This operator runbook accepts the cloud CPA failure and concurrency policy without restarting CPA, Cloudx, Phi, or any local process. It never mutates the local CPA. The transaction is bound to active signed Cloudx `0.1.21`, exact cloud `.policy.5`, and both active cloud watcher paths.
...
python3 scripts/accept_cloud_cpa_failure_policy.py \
  --apply \
  --confirm 'ACCEPT CLOUD CPA FAILURE POLICY 0.1.21'
```

Before changing the active pool, the transaction:

1. Verifies signed Cloudx `0.1.21`, exact cloud `.policy.5` path and SHA-256, cloud CPA service identity, policy/watcher readiness, one healthy active credential, an empty failure input, an absent sweep trigger, and the retained 45-entry archive.
2. Copies the active signed cloud `0.1.21` artifact into a private temporary local directory, read-only probes only the local CPA's top-level JSON snapshots through the declared mihomo HTTPS path, and requires three current real weekly-limit samples. It does not change, reload, or restart the local CPA, and removes all temporary snapshots automatically.
...
Only then does it atomically hold the one active credential and install three transaction-owned copies of the real limited samples. Natural `/v1/responses` traffic must exhaust the selectable pool; exact upstream returns its private `modelCooldownError` for this all-quota state, and `.policy.5` must recognize the error's typed capability and emit the stable identity-free `auth_unavailable` incident trigger. The active watcher must consume the trigger, probe at concurrency at least `3`—independent of the business maximum `2`—and archive zero quota credentials.
