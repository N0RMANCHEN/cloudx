...
- The private key matching current release fingerprint `SHA256:jJAS0rbecTDcU4QD3JjkJFv/5YpFyAOAtvdaf0fY4j0` is unavailable on this operator endpoint, while the only visible rejected key has a different fingerprint. Source `0.1.15` now provides `prepare_release_trust_recovery.py`: its real invocation remains `confirmation-required`, emits no key path/material, and authorizes nothing; an explicitly approved apply would generate a repository-external key and atomically synchronize only the three public roots. No rotation, commit, tag, publication, stable move, stage, activation, or restart has occurred.
- The trust-recovery path now has an end-to-end isolated regression. It rotates only a temporary source copy, builds and signs both `0.1.15` candidates and a stable index with the replacement key, verifies the old root rejects the release, and then invokes the candidate local and cloud zipapps so their embedded replacement roots accept idempotent staging. Both isolated endpoints remain selector-free. The temporary key and roots are discarded after the test; the real trust roots, refs, endpoints, services, and selectors remain unchanged and unauthorized.
...
- A strict release-ordering evidence fixture evaluates Cloudx `0.1.13`/`0.1.12` against the recorded Phi current/N-1 across four release pairs, two upgrade orders, and both rollback directions. Phi current supports both Cloudx releases directly. Source `0.1.15` packages a bounded, secret-free formal-to-legacy health bridge fixed to an immutable artifact; its generated example is accepted by the exact Phi N-1 parser at release `17d3e42`, while unknown process/failure facts remain unknown. Source now includes the isolated selector rehearsal, inactive unit installer, production-path-isolated canary, and a final overlap-first primary/legacy/primary transaction that protects selector plus gateway/importer continuity and restores the old path on failure. The audit still identifies `legacy_bridge_pending` until the bridge is signed/published and those runtime actions actually pass, so the M4A ordering checkbox remains open.
...
The ordering verification now has an explicit compatibility path rather than a declaration: `config/governance/phi_cloudx_release_ordering.v1.json` binds exact Cloudx and Phi release refs, and `scripts/check_phi_cloudx_release_ordering.py` evaluates the complete pair/order matrix. `config/governance/phi_cloudx_legacy_health_bridge.v1.json` separately binds source `0.1.15`, fixed primary/canary templates, the installer/canary/cutover contracts, the exact Phi N-1 consumer digest, and the isolated selector round trip. Its checker executes all three read-only operator plans, requires every authorization bit to remain false, requires five overlap-first cutover phases with `communicationGapAllowed=false`, and rejects any canary path capable of publishing to `/var/lib/cloudx/health`; its checkout-aware mode classified the bridge example as degraded/low-capacity rather than invalid, incompatible, or stale. The first unchecked item remains open on the same three production gates: signed artifact publication, primary bridge unit runtime acceptance, and independent runtime rollback rehearsal.
