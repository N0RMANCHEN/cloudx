...
Current-workstation `0.1.31` installation is also complete. Local selectors are `0.1.31/0.1.30`; native profile, `.zshrc`, shell source, CPA launcher/PID `15946`, loopback listener, and official Codex bytes were unchanged. Two captured Codex processes exited naturally during the observation interval, while every remaining captured PID survived and the updater managed none. A fresh shell selected API mode, resolved `/opt/homebrew/bin/codex`, and returned `CLOUDX_LOCAL_0_1_31_OK` through a real `gpt-5.6-sol` request. Both endpoints now carry the signed mixed-YAML parser fix and are ready for the separately confirmed Phase 1 retry.

Phase 1 retry is complete at transaction `20260723T063726Z-485a416b`. The immutable root-private `v0.1.31` installer changed the gateway list from five to six keys, switched the mode-`0600` client credential, updated the shadow environment to build `0985802`/deployment `shadow-0.1.31`, restarted only `cliproxy.service` as PID `1916887`, restored two watches, and retained the old key. Both current and previous credentials return HTTP `200`; unrelated key order and the mode-`0640` Phi consumer key remain unchanged. Fresh broker PID `91666` on port `28376` fetched current client-config and returned gateway HTTP `200`; handshake and fresh formal health are healthy. The real official-Codex business canary returned known `503 auth_unavailable` because formal capacity reports no available account. Treat Phase 1 business acceptance as incomplete, retain the old key, and do not run Phase 2 until a fresh real model request succeeds. Account import/promotion or CPA repair is a different Roadmap action and is not implied by this key transaction.

The operator subsequently authorized copying the local credential set to cloud. Do not import the complete local auth directory recursively: nested log/static JSON causes a safe `missing_token` rejection. A private mode-`0700` temporary export containing only the 74 top-level mode-`0600` credential JSON files passed ordinary cloud dry-run as `written=42`, `skipped=32`, applied with the same request hash and no errors, then repeated as `written=0`, `skipped=74`. These records are shadow-only. Active promotion still requires the dedicated Agent Identity transaction, whose retained source is intentionally pinned to historical signed Cloudx `0.1.26`; update, sign, and install that binding before using it on the current release.

Source `0.1.32` performs only that release rebind. `scripts/promote_cloud_agent_identity_batch.py` now requires active signed Cloudx `0.1.32` while retaining the exact policy8 binary/capability, request digest, active/new counts, private recovery bundle, cohort isolation, per-identity real canaries, restored baseline, final pool canary, idempotent repeat preview, and automatic rollback. Publish and install `0.1.32` before extracting the immutable operator script; do not run the mutable checkout copy against production.

Signed `0.1.32` is now installed cloud-first then local, with both selectors at `0.1.32/0.1.31`. Neither updater restarted or managed CPA/gateway. Cloud preserved CPA/gateway PID `1916887`, mihomo PID `277808`, restart counts `0`, transaction `20260723T063726Z-485a416b`, six keys, and one current credential match. Local preserved CPA PID `15946`, loopback listener, native auth/config, launcher, and official Codex bytes. The promotion prerequisite is now satisfied; use only the immutable tagged operator copy and exact private cohort input.

The first immutable `v0.1.32` promotion plan bound active `11`, new `12`, and request SHA-256 `384c342b61c766e4e9ada5290da0f510c855f254a36728388fc2fa59f243f3c4`, but apply correctly stopped in preflight because the sole strict pool observation is `unavailable`, not the historical `available` baseline. It created no transaction and left active count/PID unchanged. Source `0.1.33` adds a distinct `--allow-unavailable-baseline` confirmation: success still requires every cohort and final model canary to return HTTP `200`; failure restores exact baseline files/state and must reproduce HTTP `503` plus `unavailable`. Do not delete or rewrite the pool observation to bypass this gate.

Signed `0.1.33` is published and installed on both endpoints as `0.1.33/0.1.32`. The accepted identities are source `7e11bed583691bafebca8e0d99a601e82c81c433`, tag object `daf72483a33d3cfad833ccdfed367477fc38eb37`, artifact ref `8282f085c5c96f3d82d9bab44e37648955ee7619`, stable `d16fdb65c025d0841d03fd8fd3b9f49b094adb91`, and manifest `af7ea064355b71e997491d13449c389456bde38fcf3883a927e8fad8d2436be9`. Cloud CPA/gateway PID `1916887`, mihomo `277808`, active count `11`, and local CPA `15946` remained unchanged. Use only the immutable `v0.1.33` operator files for recovery promotion.

Unavailable-baseline recovery promotion is accepted at transaction `20260723T071131Z-0b669f3c`. The exact 12-record request increased active count from 11 to 23; twelve distinct Agent Identities passed twelve isolated model requests in thirteen attempts, the final combined-pool canary passed, archive remained 45, failure inputs remained empty, no sweep trigger appeared, and CPA remained PID `1916887`/restart `0`. Fresh `cloud codex --check` returned HTTP `200`; a real official-Codex `gpt-5.6-sol` request returned `CLOUDX_PHASE1_ACCEPTED_0_1_33_OK`. Phase 1 is fully accepted. Phase 2 may now proceed only through transaction `20260723T063726Z-485a416b` and its exact revocation confirmation.

Phase 2 is complete for transaction `20260723T063726Z-485a416b`. The immutable revoker removed exactly the previous digest-matched key, changed count from six to five, restarted only `cliproxy.service` as PID `1920633`, restored two watches, and recorded current-key HTTP `200`, old-key HTTP `401`, unchanged current credential, and unchanged unrelated keys. Final aggregate checks show one current mode-`0600` match, one Phi mode-`0640` match, active count `23`, mihomo PID `277808`, and root-private mode-`0600` manifest/receipt. Fresh official Codex returned `CLOUDX_PHASE2_REVOKED_OK`. Do not reuse either confirmation or transaction for a future rotation.

Signed `0.1.30` remains retained from source `ccd548db01fcf55407fb7e04ddbb76d34386722b`. Main CI `29983663602` and release workflow `29983719630` passed; annotated tag object `578765afc40e17df2c2f30cbdaf79676d04aa5c1`, artifact ref `62c7f8c5a06262ed9fef38a2d6f4ededa46071d0`, stable ref `ff861f712c705f5414be22bc19adf53ab628b2b9`, manifest SHA-256 `de6dd1ea794fb70b9dd756b333d230a19a2268ba62a82fece3b01985dda6953b`, and offline bundle SHA-256 `9d249f555ad43c5063ef74cdc1fa7cb0d5ad99e4bd70b2e84e3256382925ba0b` are accepted.

Cloud-first installation is complete. Cloud selects `0.1.30/0.1.29`, passes self-check/release-status/handshake, and preserves CPA PID `1871934`, mihomo PID `277808`, restart counts `0`, retired importer state, gateway-key count `5`, the unique private Cloudx credential, active/archive `11/46`, watcher inputs, and every relevant timer/path state. Install the current workstation separately before executing the scoped-key rotation.

Current-workstation installation is also complete. Local selectors are `0.1.30/0.1.26`; native profile, shell source, CPA launcher/PID `15946`, loopback listener, official Codex bytes, and captured Codex processes were unchanged. A fresh shell selected API mode, resolved `/opt/homebrew/bin/codex`, and returned `CLOUDX_LOCAL_0_1_30_OK` through a real `gpt-5.6-sol` request. Both endpoints are now ready for the separately confirmed scoped-key rotation.

Signed `0.1.29` is published from source `b4f45ab600e1a0a05de52c3c515c99f16037401a`. Main CI `29936168535` and release workflow `29936438372` passed; annotated tag object `ca34ec3d987910361b3e0e5d517a63be8d8fea24`, artifact ref `ef9e5a325a89f1deec0dcf7c90e7aa2de7658dbc`, stable ref `a2e211185457c64715be9a86f2d855f60c06e840`, manifest SHA-256 `05f61f978b562a6654b380f079f3fb893284a5231f953ccce421130aa5f215a6`, and offline bundle SHA-256 `09a9f3115e9ef951872fbea6e04018017c23d0bcb1d4aff2bf4367423a9ce41b` are accepted. Direct cloud-first then local upgrades selected `0.1.29/0.1.28` on both endpoints without restarting CPA; local and cloud artifact SHA-256 values are `25e6dcaa94ba395ac095e35a992cc7e8082df2da68817cc0d774d82d76a89b8f` and `272ce07da46da5f3d6c9e52dd108a2517bec4eadab3f0547324f6631413e8aa5`.

After activation, `/Users/hirohi/Desktop/sub2api-agentIdentity-alive.json` wrote and verified ten local Agent Identity records. `/Users/hirohi/Downloads/doghubx-claimed-20260722235811.json` wrote and verified ten distinct local bearer records and wrote ten records through the cloud shadow importer; both source files are private mode `0600`. Repeat local previews report ten unchanged for each source and cloud preview reports ten skipped. The local watcher retained 28 active files, including all ten Agent Identities, through immediate WatchPaths runs and two natural StartInterval observations of 120 and 121 seconds; every result reported `eligibleForArchive=0` and `archived=0`, archive count remained 39, CPA PID `72845` stayed unchanged, and final official-Codex communication passed.

Ordinary cloud import remains shadow-only and must not be described as active capacity. The separate read-only investigation is recorded in `docs/archive/2026-07-23-cloud-active-pool-timeout-diagnosis.md`. It proved that the post-import symptom was not a shadow promotion, broken listener, mihomo outage, or CPA restart: the first official-Codex websocket waited about thirty seconds and then received `auth_unavailable`, while later direct requests returned HTTP `503` immediately. The eleven-record active pool had no observed usable capacity; one bearer-eligible record produced explicit `deactivated_workspace`/later probe error evidence, and the ten Agent Identity records are intentionally outside bearer/JWT health probing. Do not promote the 45 shadow records, hold or rewrite active credentials, or restart cloud CPA without a distinct exact-confirmation transaction.

The separate M4B policy matrix is complete in `docs/archive/2026-07-23-m4b-natural-traffic-completion.md`. Forty natural websocket business sessions under the exact active policy8 process produced a maximum admitted concurrency of `2`; the retained accepted policy5 transaction proves idle zero-probe behavior, rapid natural cooldown-triggered incident concurrency `3`, quota and provisional retention, exactly one digest-matched permanent archive, and exact restore. This is policy-behavior acceptance, not a statement that the current active pool is healthy.

Signed `0.1.28` was published from source `f6048a89869f1b1e74d11b922682dd3cb1144cd5`. Main CI `29932051088` and release workflow `29932381872` passed; artifact ref `df51932a695c2b0d2bbba228e2a86332cb3466c4`, stable ref `851b217e13e58c1f7844c6d440164108ff346fda`, manifest SHA-256 `5421dc2b0d4982c46d3a498ef655984a3134468ff49866ed61ac419df6ed937f`, and offline bundle SHA-256 `3e2c0055358e952f34d101c5ca4cb50ff7fc8913e73c90e859dcee06ffd517f3` were accepted. Direct cloud-first then local upgrades selected `0.1.28/0.1.27` on both endpoints without restarting CPA. This release fixed only the official-Codex loopback canary's inherited desktop-proxy routing; the external CPA retained its configured upstream proxy and remained behind the separate recovery, five-sample zero-connection, and restart boundary.
...
Source `0.1.25` adds the distinct follow-on transaction for promoting a bounded Agent Identity batch into a non-empty active pool. Run `scripts/promote_cloud_agent_identity_batch.py` without `--apply` first, binding `--expected-request-sha256`, `--expected-active-before`, and `--expected-new`; apply requires the exact printed confirmation and receives the selected export through stdin only. The transaction requires active signed `0.1.25` plus exact policy8 capability evidence, prepares a root-only recovery copy before mutation, validates private distinct normalized files, temporarily holds only the prior baseline while the new cohort serves one real canary per imported record, then restores the baseline and requires an idempotent signed preview. Any failure restores the prior active count, transaction-owned archive/failure/sweep state, real communication, and unchanged CPA service identity. The first policy6 production attempt exposed the missing `originator: codex_cli_rs` request header; the policy7 retry then proved registration also must inherit the configured global proxy because direct cloud egress receives an unsupported-region HTTP `403`. Both were contained automatically. Policy8 pins and tests both requirements. Building, testing, committing, or publishing this tool does not authorize promotion.

The separately confirmed policy8 and promotion transactions are accepted. Policy8 activation retained backup `1784718280076915831-cloud` and selected exact SHA-256 `4dfa561451662ca5deae566f6fcfdc32bec7f42590439fa053000c4b84f915c0` as PID `1871934`, restart count `0`. An isolated Agent Identity then returned real HTTP `200` model traffic with policy `2` through the configured global proxy. Promotion transaction `20260722T111007Z-1f2d49a8` bound request SHA-256 `e86e54fbf3895cc3059cbc675312719359a0fc775d1a9f333d3b7d837cc63517`, wrote ten distinct identities beside the one-account baseline, completed ten cohort canaries in thirteen attempts, restored the baseline, passed one final combined-pool canary, and accepted an idempotent zero-write/ten-skip preview. The active pool is `11`; archive remains `45`, failure inputs and sweep trigger remain absent, health maintenance archived zero, and CPA was not restarted by promotion. Official Codex passed through the final pool. Root-private recovery for both contained predecessors and the accepted transaction remains retained.

This workstation previously selected signed Cloudx `0.1.25/0.1.24` from the complete offline bundle SHA-256 `931062f538fdc4b603fd13b132b280edb8332a18377478605c88bd3347ad5091`. Activation reported `nativeProfileChanged=false`, preserved official `/opt/homebrew/bin/codex`, the external Agent Identity CPA selection, and PID `15946`, and passed real API-profile canaries before and after. A final cloud-mode official Codex canary from local `0.1.25` passed through the eleven-account pool.

Signed Cloudx `0.1.26` is published and active cloud-first/local-second from source `f82804ee59c002c483b600c74e2e4cf52df92920`, artifact ref `17029b46cfa85478e72d3b7660fca34b54bc3aec`, stable ref `032cf2ae3131b631357f507dbaaa613897f7e4c3`, manifest SHA-256 `bfa0d85a1b81bfea3248753ae96a6987d176a343f5ad25f5cbf20979c631e1f8`, and offline bundle SHA-256 `8d1d5894fa31a96db7571d9cf96c455edaf7b5b41961c1655fbb25da826a7a5a`. Both endpoints select `0.1.26/0.1.25` and pass self-check. Local CPA PID `15946`, cloud CPA PID/restart count `1871934/0`, the eleven-record active pool, official Codex resolution, and both native profile hashes were preserved. Real official-Codex canaries returned `CLOUDX_LOCAL_UPGRADE_OK` and `CLOUDX_CLOUD_UPGRADE_OK`. The installed `codexx cloud upgrade --check --json` and `codexx upgrade --check --json` commands both verified the signed stable index and reported `up-to-date` without staging, activation, CPA management, or service restart.
...
## Published Release 0.1.21

The immutable annotated tag `v0.1.21` identifies source `f46a0e39887722bd20301270dc3e84d030f8058d`; tag object `78a0fea61120ed7780bd8ec7e784ab7bad87a3b0` peels to that exact commit. Main CI `29681741098`, non-publishing signing canary `29681791933`, and tag workflow `29681834663` passed. The immutable artifact ref is `00148d962a8e90b213b36ef5cb8fcae46a046029`; stable is `2e402391d4a5782af5602738cb2a714248f2a41f`; GitHub Release `356304310` is final and contains seven assets.

Manifest, local artifact, cloud artifact, and offline bundle SHA-256 values are respectively `fe6a1086fe1a878a3e73a0485a72c5751b606ce382e4c5da0d4d1599b8fac119`, `10370f38b3eb77c609837756a8c0f5292508ddafeace6b8a19b7700d1a83a7c7`, `0e96deebc121d5a331fc9e9ceb0621ccb330b613940050c95eb48d6da483ccc7`, and `b7122c0825d048a9740df72324d58dbff1354c752b74b5518841884a4b98c58f`. Fresh clones and authenticated downloads accepted the current signer, rejected the pre-rotation signer, matched every asset and offline member, passed both self-checks and stable selection, and staged local/cloud bundles idempotently without selectors.

Publication changed no production selector or release directory. Local and cloud remain signed `0.1.20/0.1.19`; local CPA remains PID `61859` and currently has established port-`8317` traffic, cloud CPA remains PID `1693505` with restart count `0`, importer remains PID `133756` with restart count `0`, the cloud active pool retains one credential, and failure/trigger inputs remain empty. Install cloud then local through separate recovery-bounded transactions before staging `.policy.5`; CPA activation remains a later independent restart boundary, and local activation remains prohibited until five consecutive zero-connection samples pass.

The separately confirmed cloud `0.1.21` install is accepted. Root-only transaction `20260719T094620Z-037d1c5c` captured exact selectors, CPA/importer state, credential/archive/failure/sweep tree aggregates, watcher states, unit/drop-in and private prerequisite digests, then prepared and checked one independently executable `0.1.20/0.1.19` recovery plus an automatic failure trap before mutation. The immutable tagged installer consumed the same complete offline Git bundle and returned stage `staged`, activation `active`, and `serviceRestarted=false`. Cloud now selects `0.1.21/0.1.20`; exact artifact/manifest, self-check, release status, handshake, all protected aggregates, and real local communication passed while CPA PID `1693505`, importer PID `133756`, restart counts `0`, watcher states, and all six captured Codex PIDs remained unchanged. Local remains `0.1.20/0.1.19`; its install is the next separate gate.

The separately confirmed local `0.1.21` install is also accepted. Private job `20260719T095649Z-5e63f3c8` captured native/shell/CPA/auth/process state, stored exact snapshots and the complete deploy bundle, passed the independently executable recovery `--check` with a real canary, then recaptured the final baseline and armed automatic rollback before mutation. The immutable tagged installer returned local stage `staged`, activation `active`, and `nativeProfileChanged=false`. Local now selects `0.1.21/0.1.20`; exact artifact/manifest, self-check, signed entrypoints, fresh-shell `codexx api`, and real post-install communication passed while native/shell state, CPA binary/config/LaunchAgent, the 37-file JSON aggregate, CPA PID `61859`, and all six captured Codex PIDs remained unchanged. Cloud stayed `0.1.21/0.1.20` with CPA/importer identities and restart counts unchanged. No service restarted.

Both `.policy.5` candidates are now staged and inactive after distinct exact confirmations. Cloud path `/opt/cliproxy-cloudx/releases/7.2.71-cloudx-policy.5/cli-proxy-api` is root-owned mode `0755`, size `45322402`, SHA-256 `5f83b1821d2be7cf5b7615973e4e6130d477386e16eae3a50af46e99bf7af7f8`, and reports `7.2.71-cloudx-policy.5` / `5b7f2361+cloudx-cpa-policy5`. Local path `/Users/hirohi/.local/lib/cliproxy-cloudx/releases/7.0.1-codexx-fast-service-tier-cloudx-policy.5/cli-proxy-api` is owner-only mode `0700`, size `41484978`, SHA-256 `bb6fe9cfcc26d521ce0dcf9f503d2dffa742bce62bd359cab8f91052116c0db3`, and reports the pinned local identity. Both repeated stages returned `already-staged`. Comprehensive endpoint transaction verification retained both Cloudx `0.1.21/0.1.20` selectors, cloud CPA PID `1693505`, importer PID `133756`, local CPA PID `61859`, all six captured Codex PIDs, every protected runtime aggregate, and all launcher/unit selections. Cloud `.policy.4` remains active; local remains on its original CPA binary.

The separately confirmed cloud `.policy.5` activation is accepted after two rollback-bounded invariant hardening attempts. Jobs `20260719T101124Z-114ddf79` and `20260719T101919Z-78683e87` each selected correct policy5, then rejected expected CPA auth content/cache refresh and automatically restored exact policy4 with real HTTP `200` policy-`2` traffic and no account/archive/importer/local loss. Final job `20260719T102318Z-ff41e660` locked the authoritative top-level credential set while permitting normal token-content refresh, captured and checked an independent policy4 recovery, retained canonical backup `1784456709015161519-cloud`, and restarted only `cliproxy.service`. Exact policy5 now runs as PID `1719083`, restart count `0`; health, policy `2`, one-attempt real model traffic, active credential count `1`, archive manifest count `45`, empty failure input, absent trigger, watcher states, importer PID `133756`, Cloudx selectors, and local communication all passed. The manual recovery job remains executable and restores policy4; local policy5 remains staged/inactive behind the zero-connection gate.

Cloud M4B transaction `20260719T102932Z-aee5d40f` is accepted under active policy5 without a service restart. The signed local read-only scan selected three real weekly-limited samples without mutating local CPA. Isolated quota, provisional refreshable-401, permanent non-refreshable-401, and exact-restore phases passed. Natural aggregate traffic reached cooldown within three attempts, policy5 emitted the identity-free trigger, the active watcher consumed it, and incident probing ran at concurrency `3` while business policy remained `2`; all three limited accounts were calibrated and zero quota credentials archived. Recovery restored the one useful credential and archive manifest `45`, passed real HTTP `200` policy-`2` traffic, retained CPA PID `1719083`/restart count `0`, and finished with empty failure input, absent trigger, idle probe concurrency `0`, and continuous local communication.
...
The complete ordered production procedure, immutable operator-bundle requirement, CPA continuity checks, and both recovery paths are documented in [Local Legacy Retirement Runbook](local-legacy-retirement.md).
...
The transaction then prepares a standalone restore script and moves only `~/.local/bin/codexx_app`, `~/.local/bin/codexx.py`, and `~/.local/bin/codexx-legacy` into a private retained quarantine on the same filesystem. It repeats native import, fresh-shell mode selection, selector/hook/entrypoint checks, and external CPA continuity after the move. Any failure restores every moved target before returning nonzero. Success is a quarantine receipt, not deletion: accounts, CPA binary/configuration/LaunchAgent, Cloudx entrypoints and hook, official Codex/Git, the original recovery bundle, the manual restore tool, and the quarantine all remain; no process is terminated and no service is restarted.

The replacement-workstation M5 retirement is accepted. Two unusable orphaned exec groups retired with `SIGTERM` only; the zero-connection control service moved to the verified retained runtime through one bounded port-`8765` reload; the three live package targets entered private quarantine `20260719T150428Z`; and a fourth separately confirmed transaction prepared recovery, unloaded only `com.codexx.control`, and quarantined its plist under backup `20260719T154900Z`. No live codex-plus process remains, ports `18317` and `8765` are closed, all recovery checks pass, and the local import lock is free. External CPA PID `61859` and port `8317`, signed `0.1.21/0.1.20` selectors, official Codex/Git, and real communication remained continuous.

## Quarantine Dormant Cloud Legacy Runtime

The complete cloud procedure is documented in [Cloud Legacy Runtime Retirement Runbook](cloud-legacy-runtime-retirement.md). Its transaction targets only `/opt/codex-gateway/codexx_app`, requires the five dependent legacy source files to be reachable only through the declared inactive/disabled units, verifies the complete root-only HTTP-importer runtime archive still contains that tree, and prepares a standalone root-only restore tool before the move. It executes no systemd lifecycle or reload command and cannot change the gateway, CPA, Phi, credentials, selectors, or rollback archives.

The cloud transaction is accepted from exact root-only operator bundle commit `1afa591d09b993fc35bb31507a485641a54acfce`. The 281-file tree entered quarantine `20260719T152638Z`; direct recovery and the existing importer runtime archive both verify. Gateway PID `1746294`/restart count `0`, signed `0.1.21/0.1.20`, public health/handshake, the signed bridge and Phi timers, disabled old exporter/importer/repair/quota paths, closed port `8780`, and local communication remained unchanged. No service command, daemon reload, credential mutation, or deletion occurred.
...
The default `cloudx.legacy-health-bridge-cutover-plan.v1` result reads no artifact, unit, process, selector, or health file and keeps every authorization false. Real apply requires the exact printed `CUT OVER AND REHEARSE cloudx-legacy-health-bridge WITH ROLLBACK` confirmation, root, exact signed installed bytes, the old active/enabled timer, inactive primary units, an active unchanged gateway, exact current/previous selectors, and a distinguishable old-exporter document. Importer continuity accepts either the original active process state or the separately accepted M5 retirement state: `codex-import.service` must remain inactive/disabled with PID zero and no listening or connected socket on port `8780`. The transaction cannot change the importer between those states.
...
Tokenless Sub2API `agentIdentity` input additionally requires a readable regular `cloudx.cloud-cpa-capabilities.v1` sidecar at `/etc/cloudx/cloud-cpa-capabilities.json`, or the path selected by `CLOUDX_CPA_CAPABILITY_MANIFEST`. The document names the exact absolute CPA binary, its SHA-256, runtime version, and `codex-agent-identity-v1`. `CLOUDX_CPA_BINARY` may pin the expected executable path. Every preview and write re-hashes that binary and requires the same capability in the configured gateway `/healthz` response header; stale or absent evidence returns `external_capability_missing` before creating import target state. Installing or restarting a compatible cloud CPA remains a separate operator-confirmed action.
...
The M5 stop transaction is now accepted with fresh evidence digest `sha256:8b29a28bc6f3f1109d26e95344de041f951e1a17e134319c5d8748c6bcdc3104`. `codex-import.service` is loaded but inactive/dead and disabled, PID `0`, with port `8780` closed. SSH import dry-run, formal Cloudx health, Phi formal-health consumer, and gateway/model canaries passed; gateway policy5 PID `1719083`, Cloudx `0.1.21/0.1.20`, selectors, and local communication remained unchanged. Runtime, unit/drop-ins, token metadata, failure receipts, legacy exporter, and rollback snapshot `/var/lib/cloudx/http-importer-stop-prep/20260716T075313Z` remain retained. This is reversible retirement, not package or evidence deletion.
...
Production transaction `20260719T090507Z-14915b32` then proved `.policy.4` still could not emit the natural cooldown trigger. Its isolated quota/provisional/permanent/restore phases passed, the aggregate phase rejected with `aggregate_not_triggered`, and automatic recovery restored one useful credential, all 45 archive entries, empty watcher inputs, HTTP `200` policy-`2` traffic, cloud CPA PID `1693505` with restart count `0`, and local CPA/Codex continuity. Exact upstream returns private `modelCooldownError`, so `.policy.4`'s preceding `*coreauth.Error` cast made its later string-code branch unreachable; the synthetic test had constructed an error shape the selector never returns. Source `.policy.5` adds an explicit capability to the real selector type and checks it before that cast. The shared patch SHA-256 is `2101b12519607ab022fd0780069d3f75207a1d2046114450420df4906dc0ded8`; three independent builds match local SHA-256 `bb6fe9cfcc26d521ce0dcf9f503d2dffa742bce62bd359cab8f91052116c0db3`, size `41484978`, and cloud SHA-256 `5f83b1821d2be7cf5b7615973e4e6130d477386e16eae3a50af46e99bf7af7f8`, size `45322402`. They are not yet staged or active.

Do not invoke synchronous local activation from a Codex turn that is itself using the local CPA. The next local `.policy.5` activation requires signed Cloudx `0.1.21` so the independently executable recovery tool, safe stage receipts, and zero-established-connection gate are present before another restart is authorized. After that release is active locally, inspect the non-authorizing deferred plan:

```bash
python3 scripts/schedule_local_cpa_policy_activation.py
```

Its exact-confirmation apply prepares the installer, pinned contract, original launcher snapshot, recovery tool, `RECOVERY.txt`, and digest-bound job manifest before starting the private worker. After the 180-second delay, the worker may wait up to seven days using only one bounded socket audit per minute. It invokes the installer only after five consecutive audits report zero established CPA connections; the installer repeats the same gate before editing the launcher or stopping CPA. Expiry changes no service or launcher. Automatic recovery invokes the same job-local command documented for the operator, waits for complete launchd unloading, retries baseline bootstrap, and requires both health and real Codex communication. Worker logs contain only enumerated stages and aggregate states. The detailed procedure and failure-code response are in [Local CPA Activation And Recovery Manual](local-cpa-recovery.md).

Production job `20260719T160250Z-9a24174c` is now in `quiescence-wait` from immutable commit `ec24256dbe10c83833818e9e58c7e8f7967b368a`. Its independent recovery plan accepted baseline PID `61859`; 66 established rows remained when monitoring began, so no launcher or service state changed. The worker may proceed only after natural zero-connection evidence and the installer's repeated gate. Do not create a competing activation job or manually stop Codex processes while it is active.

A later replacement-workstation audit found that the running baseline still has SHA-256 `cf9641b3e50ae486aec1698dec88f735589680f9ae98558c29cde184daac3a96`, advertises no Agent Identity capability, and has no local capability sidecar. The copied policy5 worker cannot mutate this newer endpoint: its installer requires active signed Cloudx `0.1.21`, while the endpoint now selects `0.1.26`. Source `0.1.27` therefore supersedes—not edits or bypasses—that historical job with combined local candidate `7.0.2-codexx-fast-service-tier-cloudx-policy.9-agent-identity`, SHA-256 `174a46d58a95f56104d0bb3722c4fb5e7dffc125f2f525505d96f556291aa761`, size `41518146`. The candidate composes the two-request, failure-receipt, aggregate-sweep, fast-tier, Agent Identity, and live capability contracts on exact upstream `v7.0.2`.

Publication and local installation of signed Cloudx `0.1.27` were prerequisites for staging. Stage is side-by-side and process-neutral. A new recovery-bounded activation must again wait for five natural zero-connection samples and use a new exact confirmation; it may not reuse the old policy5 receipt or follower. After restart, the installer requires the capability header, policy `2`, and real official-Codex traffic before atomically writing `~/.local/bin/cli-proxy-api.capabilities.json` as mode `0600`. Failure restores the original launcher and the prior sidecar snapshot. Do not create the sidecar manually: the digest plus live header are the import authorization boundary.

Signed `0.1.27` rollout is complete and the exact combined candidate is now staged under `~/.local/lib/cliproxy-cloudx/releases/7.0.2-codexx-fast-service-tier-cloudx-policy.9-agent-identity`. Repeated staging is idempotent and left the active launcher, original binary, PID `61859`, listener, connections, and absent sidecar unchanged. New activation job `20260722T144535Z-564eaabf` prepared its private recovery bundle before entering quiescence wait; a distinct follower with the same job ID waits for an accepted activation and communication receipt before invoking the watcher. The first audit observed the current persistent `ClashX Pro`/CPA connection, returned `busy`, and performed no service action. Do not terminate that connection or manually invoke the copied installer.

After signed 0.1.28 removed inherited desktop-proxy routing from the canary, job `20260722T151933Z-0db72bf9` passed all five zero-connection samples but stopped before launcher mutation because the old bearer-only pool returned exact `auth_unavailable`; the official native profile also imported and verified structurally but real traffic exposed the same unusable state. Use the scheduler's `--bootstrap-agent-identity <absolute-path>` only for this explicit cold-start condition. Apply binds the source SHA-256 without copying raw input into the job, starts the staged capable CPA only after recovery/quiescence gates, uses the active release's importer and its normal lock/validation/atomic-write contract, and still requires the final official-Codex success marker before retaining the launcher and capability sidecar. Any failure restores transaction-changed auth JSON, sidecar, and baseline selection.

Cold-bootstrap job `20260722T154032Z-d2f17f2d` is accepted. Candidate PID `72845` selects the staged policy9 Agent Identity binary; live health, capability, policy `2`, ten-record import/verification, final official-Codex communication, sidecar mode/digest, original-baseline retention, and recovery-plan inspection all passed. Repeating `codexx import /Users/hirohi/Desktop/sub2api-agentIdentity-alive.json` is idempotent with ten unchanged records. Its separately authorized follower also succeeded: `com.codexx.cliproxy-refresh` watches both the private failure directory and exact aggregate trigger, retains `StartInterval=120`, completed its first run with exit `0`, and did not restart CPA.

The first live fallback cycles exposed a local consumer defect after that acceptance: valid tokenless Agent Identity files were classified by the bearer-only missing-token rule and archived. Source `0.1.29` makes the watcher credential-aware. A well-formed Agent Identity is retained without synthetic bearer tokens; a malformed one is archived only as `invalid-agent-identity`; and the aggregate usage sweep skips Agent Identity because that probe requires a bearer JWT. A fresh, permanent CPA runtime receipt may still archive one Agent Identity only when its auth filename and current SHA-256 match exactly. Do not restore affected identities under an older active Cloudx release, because its next 120-second maintenance cycle can repeat the misclassification.

The later explicitly authorized restart attempt identified a canary ordering defect without changing the launcher. Quitting ClashX made the direct quiescence check pass, but also removed the CPA's required upstream proxy; reopening it restored upstream access while the official-Codex canary inherited desktop proxy variables and routed its loopback CPA request back through ClashX, recreating the established connection. Source `0.1.28` removes inherited `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY` and lowercase equivalents from only the canary subprocess and sets `NO_PROXY`/`no_proxy` to `127.0.0.1,localhost,::1`. It does not alter ClashX, system proxy configuration, CPA launcher/configuration, or CPA outbound proxy behavior. Publish and install the signed fix before creating a new activation job; the copied `0.1.27` job must not be reused.
...
The matching signed Cloudx receipt/sweep consumers and the currently approved endpoint policy producer must be active before event-driven maintenance is accepted. On the replacement workstation this now means the superseding `.policy.9-agent-identity`; the old policy5 follower cannot authorize the new watcher. Inspect the separate non-authorizing watcher plans only after those prerequisites pass:
