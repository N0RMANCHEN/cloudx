...
Ordinary cloud import remains shadow-only and must not be described as active capacity. After this import, formal health reported gateway healthy, importer ready, and one existing active account available, but both an official-Codex request and a direct non-streaming `/responses` request against that active pool timed out. Treat that as a separate active-pool investigation. Do not promote the doghubx shadow records or restart cloud CPA without its own explicit rollback-bounded approval.
