...
- The initial Phi Personal Agent Mesh topology is frozen by the authoritative product contract and a machine-checked governance profile. The signed cloud artifact publishes the read-only compatibility profile, the gateway-only credential policy, and `cloudx.phi-cloud-consumer-traffic-policy.v1`. The traffic policy bounds the single consumer to four in-flight logical requests, a sixteen-entry FIFO queue, thirty gateway attempts per minute with burst four, separated timeouts, and at most three attempts without adding Task or device concepts. These documents install no credential or limiter and authorize no runtime, service, release, restart, or activation change.
