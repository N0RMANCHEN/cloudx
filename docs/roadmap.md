...
- Repository development remains at `0.1.14` after immutable signed `0.1.13` activated cloud-first and local-second, reconciled the local hook through the newly current updater, and completed a dual-endpoint N-1 rollback round trip. Source `0.1.14` retains the alias-root N-1 regression fix for a later release.
...
- The goal-watchdog migration removes its legacy health-contract signal dependency, but a refreshed read-only importer audit found new cloud-host `GET /v1/accounts` and `POST /v1/imports` requests on `2026-07-16` between `08:36` and `08:39` CST. `codex-import.service` therefore remains enabled and active on port `8780`; caller attribution, a fresh quiet-traffic gate, rollback readiness, and a separate explicit stop decision are still required. The legacy health exporter remains active as rollback evidence.
