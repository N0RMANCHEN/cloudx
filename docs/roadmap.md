...
Status: signed Cloudx `0.1.17` is active on cloud and local with signed N-1 retained. The staged `.policy.1` candidates and never-staged `.policy.2` source candidates are superseded. Cloud `.policy.3` is active after a successful exactly confirmed retry; local `.policy.3` remains durably staged and inactive. Local activation and both watcher transactions remain separately unapproved. The revised policies separate the two-request business ceiling from trigger-only high-concurrency incident diagnosis, retain the immediate direct-permanent receipt path, and add identity-free aggregate pool observations.
