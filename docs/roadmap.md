...
- The initial Phi Personal Agent Mesh topology is frozen by the authoritative product contract and a machine-checked governance profile. The signed cloud artifact publishes both the read-only `cloudx.phi-mesh-compatibility-profile.v1` reference set and the secret-free `cloudx.phi-cloud-consumer-credential.v1` policy. The latter defines a distinct gateway-only, revocable, overlap-rotatable Phi cloud bearer with no device representation, SSH, import, gateway-mutation, or release authority. Neither document installs a credential or authorizes a runtime, service, release, restart, or activation change.
