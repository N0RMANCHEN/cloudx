...
- Repository development has advanced to `0.1.10`; its cloud candidate contains the native CPA auth scanner, quota probe, atomic quarantine, explicit restore path, and matching recovered trust root in both endpoint artifacts. Deployed endpoints still run signed `0.1.8` and retain signed `0.1.7` for rollback.
...
- M5 dependency preflight still finds the legacy `18317` listener, local CPA, existing Codex sessions, old cloud importer, legacy health exporter, and `/opt/codex-gateway/codexx_app` compatibility runtime. The installed signed `0.1.8` CPA-health unit still loads that runtime. Immutable `0.1.9` was published but its candidate cloud staging rejected a stale embedded trust root before writing a cloud release; stable was restored to signed `0.1.8`, neither endpoint activated `0.1.9`, and repository `0.1.10` fixes the mirrored trust root. The quota-monitor timer was disabled by a concurrent external change outside the Cloudx cutover. No deletion gate is accepted yet.
...
- The `/Users/hirohi` macOS workstation has now been independently migrated from its codex-plus-only local entrypoint to signed `0.1.8`, with signed `0.1.7` as `previous`. Its external local CPA remains PID `38189` on `127.0.0.1:8317`, all pre-existing Codex processes survived activation and rollback, and the private codex-plus recovery command remains healthy; this does not advance the M5 deletion gate.
...
- [ ] Publish and activate the signed native CPA-health release, replace only its unit in a separately confirmed maintenance action, and accept repeated observation before retiring `/opt/codex-gateway/codexx_app`.
