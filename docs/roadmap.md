...
- A strict release-ordering evidence fixture evaluates Cloudx `0.1.13`/`0.1.12` against the recorded Phi current/N-1 across four release pairs, two upgrade orders, and both rollback directions. Phi current supports both Cloudx releases directly. Source `0.1.15` packages a bounded, secret-free formal-to-legacy health bridge fixed to an immutable artifact; its generated example is accepted by the exact Phi N-1 parser at release `17d3e42`, while unknown process/failure facts remain unknown. A deterministic isolated rehearsal drove the real selector transaction through `.13/.12 -> .12/.13 -> .13/.12`, and a separate exact-confirmation installer can now place only the signed candidate unit files while leaving them disabled/inactive and the old exporter active. The audit still identifies `legacy_bridge_pending` until the bridge is signed/published, installed and runtime-accepted, and production rollback-rehearsed, so the M4A ordering checkbox remains open.
...
The ordering verification now has an explicit compatibility path rather than a declaration: `config/governance/phi_cloudx_release_ordering.v1.json` binds exact Cloudx and Phi release refs, and `scripts/check_phi_cloudx_release_ordering.py` evaluates the complete pair/order matrix. `config/governance/phi_cloudx_legacy_health_bridge.v1.json` separately binds source `0.1.15`, the fixed-artifact templates, the non-activating unit installer contracts, the exact Phi N-1 consumer digest, and the accepted isolated selector round trip. Its checker executes the installer's read-only plan and requires every authorization bit to remain false; its checkout-aware mode classified the bridge example as degraded/low-capacity rather than invalid, incompatible, or stale. The first unchecked item above remains open on three explicit production gates: signed artifact publication, bridge unit runtime acceptance, and independent runtime rollback rehearsal.
