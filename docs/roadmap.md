...
- Signed `0.1.16` carries the first separately gated CPA credential-failure and concurrency safety batch. Its `.policy.1` exact-commit candidates remain durably staged but inactive at `/Users/hirohi/.local/lib/cliproxy-cloudx/releases/7.0.1-codexx-fast-service-tier-cloudx-policy.1` and `/opt/cliproxy-cloudx/releases/7.2.71-cloudx-policy.1`; they are now superseded and must not be activated. Source `0.1.17` retains the process-global maximum of two proxied API requests and local fast-service-tier mapping while adding infrastructure-gated, concurrency-one account probing, bounded `deactivated_workspace` classification, and immediate digest-bound archive after one conclusive permanent result. Independently rebuilt `.policy.2` candidates match local SHA-256 `f288838053f43a82c50d2ab23bcb096c627a848fdf662413544a483f908f236d` and cloud SHA-256 `7c9603a380f9fbd7bdbe1c8ecbf938504f6055677ba4d4de2cd7004398a02229`; neither is staged or active, and both require signed Cloudx `0.1.17` before activation.
...
- [x] Add an infrastructure/provider gate, explicit external proxy input, concurrency-one account probe, bounded permanent-error parsing, direct conclusive archive, and Phi read-only boundary; retain quota/429, refreshable 401, network/TLS/DNS/timeout/5xx, and unknown failures.
