...
- A strict release-ordering evidence fixture evaluates Cloudx `0.1.13`/`0.1.12` against the recorded Phi current/N-1 across four release pairs, two upgrade orders, and both rollback directions. Phi current supports both Cloudx releases directly. Source `0.1.15` packages a bounded, secret-free formal-to-legacy health bridge fixed to an immutable artifact; its generated example is accepted by the exact Phi N-1 parser at release `17d3e42`, while unknown process/failure facts remain unknown. A deterministic isolated rehearsal then drove the real selector transaction through `.13/.12 -> .12/.13 -> .13/.12`; the fixed candidate stayed independent and emitted identical bytes in all three states. The audit still identifies `legacy_bridge_pending` until the bridge is signed/published, installed side-by-side, and production rollback-rehearsed, so the M4A ordering checkbox remains open.
...
The ordering verification now has an explicit compatibility path rather than a declaration: `config/governance/phi_cloudx_release_ordering.v1.json` binds exact Cloudx and Phi release refs, and `scripts/check_phi_cloudx_release_ordering.py` evaluates the complete pair/order matrix. `config/governance/phi_cloudx_legacy_health_bridge.v1.json` separately binds source `0.1.15`, the fixed-artifact templates, the exact Phi N-1 consumer digest, and the accepted isolated selector round trip. Its checkout-aware checker classified the example as degraded/low-capacity rather than invalid, incompatible, or stale. The first unchecked item above remains open on three explicit production gates: signed artifact publication, bridge unit acceptance, and independent runtime rollback rehearsal.

The privileged-boundary verification now has the same fail-closed form: `config/governance/phi_cloudx_privileged_boundary.v1.json` binds exact deployed Phi refs to a secret-free credential, identity, and Agent-surface permission snapshot, and `scripts/check_phi_cloudx_privileged_boundary.py` derives effective Cloudx auth/import/gateway/release authority. Current interactive and mail surfaces remain blocked by reachable unrestricted elevation and the non-scoped administrative gateway credential; the `NoNewPrivileges`-confined orchestrator evaluates safe. The second unchecked item above is intentionally not marked complete, and this repository batch authorizes no Phi or Cloudx runtime mutation.
