...
8. Reversibly archive unusable local CPA credentials with `codexx api refresh --apply` after strict permanent-auth evidence, while retaining an exact-confirmation restore path.
...
- A CPA policy candidate may be built only from the exact deployed upstream revision and activated only through a separate operator-confirmed external-service restart. It enforces one global two-request inference ceiling and may emit private, digest-bound failure receipts; it never moves or deletes an auth file itself.
- Automatic CPA archive requires at least two fresh pieces of evidence, including a conclusive permanent authentication failure, and revalidates the exact active auth-file digest before a same-filesystem move. Weekly allowance exhaustion, ordinary HTTP 429, transient rate limiting, network failure, timeout, 5xx, or an expired access token with a refresh token are never archive authority. Archive is reversible and is not credential deletion.
